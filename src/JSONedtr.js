...
//Default config values
    ...
    JSONedtr.level = function (node, lvl = 0, isArrayParam ) {
        ...
        var keyInputClasses = JSONedtr.getConfigValue('keyInputClasses', null);
        var valueInputClasses = JSONedtr.getConfigValue('valueInputClasses', null);
...
//console.log('key', key, 'value', value, 'type', type);
            ...
            var keyClasses = [ 'jse--key' ];
            if ( keyInputClasses ) { keyClasses.push(keyInputClasses); }
            var valueClasses = [ 'jse--value' ];
            if ( valueInputClasses ) { valueClasses.push(valueInputClasses); }
            if (type === 'object') { /* Object || Array */
                ...
                output += '<div class="jse--row ' + rowClass + '" id="' + rowId + '"><input type="text" class="'+keyClasses.join(' ')+'" data-level="' + lvl + '" value="' + key + '" placeholder="'+lblKey+'" '+keyReadOnly+' /> <span class="jse--typeof" data-type="' + type + '">(' + labelText + ')</span>';
                ...
                output += '<div class="jse--row '+rowClass+'" id="' + rowId + '"><input type="text" class="'+keyClasses.join(' ')+'" data-level="' + lvl + '" value="' + key + '" '+keyReadOnly+' placeholder="'+lblKey+'" /> <span class="jse--typeof" data-type="' + type + '">(' + labelText + ')</span><input type="' + inputValueType + '" class="'+valueClasses.join(' ')+'" value="' + value + '" ' + checked + ' placeholder="'+lblValue+'" '+readOnlyProp+' '+disabled+' /><div class="jse--delete">✖</div></div>';
            }
        })
...
//console.log(plusO);  
        ...
        var valueInputClasses = JSONedtr.getConfigValue('valueInputClasses', null);
        var valueClasses = [ 'jse--value', 'jse--value__new' ];
        if ( valueInputClasses ) { valueClasses.push(valueInputClasses); }
        
        plusO.html('<input type="text" class="'+ keyClasses.join(' ') +'" data-level="' + lvl + '" value="" placeholder="'+lblKey+'" /> <span class="jse--typeof">( ' + typeofHTML + ' )</span><input type="text" class="'+ valueClasses.join(' ')+'" value="" placeholder="'+lblValue+'" /><button class="jse--save">' + lblSave + '</button><button class="jse--cancel">' + lblCancel + '</button>');
        ...
        plusO.find('select.jse--typeof-list').change(function () {
            ...
            switch (o.val()) {
                case 'number':
                    p.siblings('.jse--value__new').replaceWith('<input type="number" class="'+ valueClasses.join(' ') +'" value="" placeholder="'+lblValue+'" />');
                    p.siblings('.jse--value__new').focus();
                    break;
                case 'string':
                    p.siblings('.jse--value__new').replaceWith('<input type="text" class="'+ valueClasses.join(' ') +'" value="" placeholder="'+lblValue+'" />');
                    p.siblings('.jse--value__new').focus();
                    break;
                case 'boolean':
                    p.siblings('.jse--value__new').replaceWith('<input type="checkbox" class="'+ valueClasses.join(' ') +'" value="1" />&nbsp;&nbsp;&nbsp;');
                    p.siblings('.jse--value__new').focus();
                    break;
                case 'array':
                case 'object':
